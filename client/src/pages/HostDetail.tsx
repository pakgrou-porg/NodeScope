    ...
    <DashboardLayout>
      <div className="mx-auto max-w-[1680px] p-6 xl:p-8">
        ...
        <Tabs defaultValue="hardware" className="mt-6">
          ...
          <TabsContent value="history" className="mt-6"><section className="rounded-2xl border border-white/8 bg-[#0b1924] p-5"><SectionHeader icon={Network} title="Five-minute history" note="Source samples are retained only under the configured raw-retention and rollup policy." /><div className="h-[340px]"><ResponsiveContainer width="100%" height="100%"><AreaChart data={host.history} margin={{ top: 10, right: 8, bottom: 0, left: -18 }}><defs><linearGradient id="throughput" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#67e8f9" stopOpacity={0.35} /><stop offset="100%" stopColor="#67e8f9" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="rgba(255,255,255,.06)" vertical={false} /><XAxis dataKey="timestamp" tickFormatter={(value) => new Date(value).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} tick={{ fill: "#64748b", fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: "#64748b", fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: "#0b1924", border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, fontSize: 12 }} labelFormatter={(value) => new Date(value).toLocaleString()} /><Area type="monotone" dataKey="throughput" stroke="#67e8f9" strokeWidth={2} fill="url(#throughput)" name="Generation throughput" /><Area type="monotone" dataKey="cpu" stroke="#818cf8" strokeWidth={1.5} fill="transparent" name="CPU utilization" /></AreaChart></ResponsiveContainer></div></section></TabsContent>
        </Tabs>
      </div>
    </DashboardLayout>
  );
