...
import { Activity, AlertTriangle, ArrowLeft, Container, Cpu, Database, HardDrive, MemoryStick, Network, SearchCheck, ThermometerSun, Zap } from "lucide-react";
...
export default function HostDetail({ hostId, preview = false }: { hostId: string; preview?: boolean }) {
  ...
  const hostAlertsQuery = trpc.nodescope.alerts.forHost.useQuery({ hostId }, { enabled: !preview, refetchInterval: 5_000 });
  const host = preview ? previewQuery.data?.hosts.find((candidate) => candidate.id === hostId) : hostQuery.data;
  const hostAlerts = preview ? previewQuery.data?.alerts.filter((alert) => alert.hostId === hostId) ?? [] : hostAlertsQuery.data ?? [];
      ...
      <div className="mx-auto max-w-[1680px] p-6 xl:p-8">
        ...
        <Tabs defaultValue="hardware" className="mt-6">
          ...
          <TabsContent value="containers" className="mt-6"><InventoryPanel icon={Container} title="Container inventory" note="All discovered containers are displayed; alerts remain limited to selected containers." rows={host.containers.map((container) => ({ primary: container.name, secondary: `${container.image} · ${container.age}`, state: container.state === "running" ? "healthy" : container.state === "restarting" ? "degraded" : "unavailable", selected: container.selected }))} /></TabsContent>
              ...
              <TabsContent value="alerts" className="mt-6"><HostAlertSummaryPanel alerts={hostAlerts} /></TabsContent>
...
type HostAlert = { id: string; severity: "critical" | "warning" | "info"; state: "active" | "acknowledged" | "resolved"; title: string; detail: string; observedAt: Date | string };

const alertSeverityStyles: Record<HostAlert["severity"], string> = {
  critical: "border-rose-300/20 bg-rose-400/10 text-rose-100",
  warning: "border-amber-300/20 bg-amber-300/10 text-amber-100",
  info: "border-cyan-300/15 bg-cyan-300/10 text-cyan-100",
};

export function HostAlertSummaryPanel({ alerts }: { alerts: HostAlert[] }) {
  const activeAlerts = alerts.filter((alert) => alert.state === "active");
  return <section className="max-w-5xl rounded-2xl border border-white/8 bg-[#0b1924] p-5"><SectionHeader icon={AlertTriangle} title="Host alerts" note="Automatic state changes evaluate fresh metric evidence only; stale, unavailable, estimated, and experimental evidence cannot open an alert." /><div className="mb-4 rounded-xl border border-cyan-300/10 bg-cyan-300/[0.045] px-4 py-3 text-[11px] leading-5 text-cyan-100">{activeAlerts.length === 0 ? "No active alerts for this host." : `${activeAlerts.length} active ${activeAlerts.length === 1 ? "alert requires" : "alerts require"} operator attention.`} Acknowledgement is an operator workflow state and does not change the evidence-quality guard.</div>{alerts.length === 0 ? <div role="status" className="rounded-xl border border-dashed border-white/10 bg-white/[0.02] px-4 py-5"><p className="text-xs font-medium text-slate-300">No host alerts recorded</p><p className="mt-2 text-[11px] leading-5 text-slate-500">No active or acknowledged alert is available for this host in the current console view.</p></div> : <div className="overflow-hidden rounded-xl border border-white/7">{alerts.map((alert) => <div key={alert.id} className="border-b border-white/6 px-4 py-4 last:border-0"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-medium text-slate-200">{alert.title}</p><p className="mt-1 max-w-3xl text-[11px] leading-5 text-slate-500">{alert.detail}</p></div><div className="flex shrink-0 items-center gap-2"><span className={cn("rounded-full border px-2 py-0.5 text-[9px] font-medium uppercase tracking-[0.08em]", alertSeverityStyles[alert.severity])}>{alert.severity}</span><span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[9px] font-medium uppercase tracking-[0.08em] text-slate-300">{alert.state}</span></div></div><p className="mt-3 text-[10px] text-slate-600">Observed {new Date(alert.observedAt).toLocaleString()}</p></div>)}</div>}</section>;
}
