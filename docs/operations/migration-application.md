...
The script then requires the dedicated NodeScope migrator credentials, verifies both NodeScope logins remain denied from the disposable sibling fixture, runs the requested SQL inside a rolled-back preflight transaction, applies the migration only after those gates pass, and runs the post-apply shared-project isolation verification. Do not invoke `psql` directly for production migrations.
