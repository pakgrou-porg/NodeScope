...
func TestLoadConfigRejectsUnsafeOrDuplicateReplicaEndpoints(t *testing.T) {
	for name, mutate := range map[string]func(map[string]string){
		...
		"leading-zero port alias": func(values map[string]string) { values["NODESCOPE_SECONDARY_ENDPOINT"] = "https://10.116.2.145:08443" },
		"implicit default port alias": func(values map[string]string) {
			values["NODESCOPE_PRIMARY_ENDPOINT"] = "https://framework.example.lan"
			values["NODESCOPE_SECONDARY_ENDPOINT"] = "https://framework.example.lan:443"
		},
		"path prefix trailing slash alias": func(values map[string]string) {
			values["NODESCOPE_PRIMARY_ENDPOINT"] = "https://framework.example.lan:8443/nodescope"
			values["NODESCOPE_SECONDARY_ENDPOINT"] = "https://framework.example.lan:8443/nodescope/"
		},
...
func TestLoadConfigRejectsLoopbackReplicaEndpointsOutsideExplicitDevelopmentMode(t *testing.T) {
	for name, mutate := range map[string]func(map[string]string){
		...
		"normalized localhost primary": func(values map[string]string) {
			values["NODESCOPE_PRIMARY_ENDPOINT"] = "https://localhost.:8443"
		},
		...
		"IPv4-mapped IPv6 primary": func(values map[string]string) {
			values["NODESCOPE_PRIMARY_ENDPOINT"] = "https://[::ffff:127.0.0.1]:8443"
		},
