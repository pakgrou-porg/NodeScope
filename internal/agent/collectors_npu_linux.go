...
func (collector *XDNACollector) Collect(ctx context.Context, observedAt time.Time) ([]telemetry.Sample, error) {
	...
	if err != nil {
		return []telemetry.Sample{
			unavailableSample("npu-0", "npu.availability", "state", "xrt-smi", "xrt-smi is not installed or not on PATH", observedAt),
			unavailableSample("npu-0", "npu.utilization", "percent", "xrt-smi", "NPU utilization is unavailable without a supported structured collector", observedAt),
		}, nil
	}
	...
	if err := exec.CommandContext(probe, binary, "examine", "--batch").Run(); err != nil {
		return []telemetry.Sample{
			unavailableSample("npu-0", "npu.availability", "state", "xrt-smi", "xrt-smi did not complete a readable NPU examination", observedAt),
			unavailableSample("npu-0", "npu.utilization", "percent", "xrt-smi", "NPU utilization is unavailable without a supported structured collector", observedAt),
		}, nil
	}
	return []telemetry.Sample{
		numericSample("npu-0", "npu.availability", "state", 1, "xrt-smi", "xrt-smi reports an accessible AMD XDNA NPU", observedAt),
		unavailableSample("npu-0", "npu.utilization", "percent", "xrt-smi", "NPU utilization is unavailable until a structured xrt-smi metric contract is configured", observedAt),
	}, nil
}
