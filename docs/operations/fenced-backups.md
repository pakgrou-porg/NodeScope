...
The Framework and Asus replicas both attempt the scheduled job. A database-time fenced lease named `daily_backup` authorizes only one replica to publish a snapshot. The publisher rechecks its fencing token immediately before the final atomic rename, preventing a stale or partitioned replica from presenting a backup as current.
